...
**Project**: WPF-MCP Server for AI Agent Integration
**Review Status**: Expert Panel Reviewed (Wiegers, Adzic, Fowler, Nygard, Crispin)
...
### 1.1 Vision
Create a Model Context Protocol (MCP) server that enables AI agents (Claude, GPT, etc.) to programmatically interact with Windows Presentation Foundation (WPF) applications. This server will be analogous to Playwright MCP for web browsers, but specifically designed for WPF desktop applications.
...
| **Reliability** | 99%+ success rate for standard UI operations on automation-ready WPF apps (see Section 2.3) |
...
### 2.3 Definition: Automation-Ready WPF Application
...
**Note**: Applications not meeting these criteria will have degraded automation success rates. See Appendix D for automation readiness checklist.
...
**Deliverables**:
- Project setup with .NET 8 and MCP SDK
- FlaUI integration and basic automation wrapper
- Application launch/attach functionality
- Basic element discovery (find by AutomationId)
- stdio transport working with Claude Code

**Success Criteria**:
- Can launch a WPF app and get main window reference
- Can find elements by AutomationId
- Claude Code can connect and list tools

### Phase 2: Observation Tools (Week 3)
**Deliverables**:
- `wpf_snapshot` with accessibility tree serialization
- Element reference system implementation
- `wpf_find_element` with multiple criteria
- `wpf_get_element_properties`

**Success Criteria**:
- LLM can receive structured element tree
- Element references work for subsequent interactions
- Property inspection works for all common control types

### Phase 3: Interaction Tools (Week 4)
**Deliverables**:
- `wpf_click`, `wpf_type`, `wpf_set_value`
- `wpf_toggle`, `wpf_select`
- `wpf_expand_collapse`
- `wpf_press_key`

**Success Criteria**:
- Can perform all standard UI interactions
- Proper error handling for unsupported patterns
- Actions return meaningful feedback

### Phase 4: Navigation & Polish (Week 5)
**Deliverables**:
- Scroll and focus tools
...
- Ready for autonomous development testing
...
|-------------|--------|-------------|
| Tool coverage | 20+ MCP tools | Count of implemented tools |
| Control type support | 15+ WPF controls | Controls with working automation |
| Pattern support | 10+ UI Automation patterns | Implemented patterns |
| Response time | See Performance Baselines (6.4) | P95 latency per operation type |
...
|------|-------------|--------|------------|
| **Custom controls without AutomationPeers** | High | Medium | Document requirements for target apps; provide guidance |
| **Cross-process performance** | Medium | Medium | Implement element caching; batch operations |
| **Thread synchronization issues** | Medium | High | Use dedicated automation thread; proper async patterns |
| **Stale element references** | High | Low | Clear documentation; auto-refresh on errors |
| **WPF app not designed for automation** | High | High | Provide best practices guide; graceful degradation |

### 7.1 Failure Modes Specification
...
- Element reference encoding/decoding
- Accessibility tree serialization
- Pattern detection logic

### 9.2 Integration Tests
- End-to-end tool invocations against sample WPF app
- Multi-window scenarios
- Error condition handling
- Timeout behavior

### 9.3 AI Agent Testing
- Autonomous task completion with Claude Code
...
- **WinForms support**: Extend automation to WinForms applications
- **UWP/WinUI support**: Modern Windows app automation
- **Recording mode**: Capture user interactions for playback
- **Visual element targeting**: Optional coordinate-based interactions
- **Multi-app orchestration**: Control multiple applications simultaneously
- **Event subscriptions**: Real-time UI change notifications

### 10.2 Integration Opportunities
- **Claude Desktop integration**: Native MCP server registration
- **CI/CD pipelines**: Automated WPF testing in build processes
- **Test framework adapters**: xUnit, NUnit integration

---

## Appendix A: Sample WPF Application Requirements
...
## Appendix B: Accessibility Tree Format Specification
...
## Appendix D: Automation Readiness Checklist
...
Run the WPF-MCP validation command to check your application:
```bash
wpf-mcp validate --process "YourApp"
```

Output includes:
- Elements missing AutomationId
- Controls without proper peers
- Accessibility tree depth analysis
- Performance recommendations

---

## Appendix E: Concrete Workflow Examples

### Example 1: Login Form Automation

```gherkin
Feature: Login Form Automation

  Scenario: Successful login workflow
    Given the application "CustomerApp.exe" is launched
    When the AI agent calls wpf_snapshot
    Then the response contains:
      """
      - window "Login" [ref=e1]
        - textbox "Username" [ref=e2] [value=""]
        - textbox "Password" [ref=e3] [value=""]
        - button "Login" [ref=e4]
        - checkbox "Remember Me" [ref=e5] [unchecked]
      """

    When the AI agent calls wpf_type:
      | ref | text  |
      | e2  | admin |
    Then the response is:
      """json
      {"success": true, "data": {"characters_typed": 5}}
      """

    When the AI agent calls wpf_type:
      | ref | text      |
      | e3  | secret123 |
    And the AI agent calls wpf_click:
      | element       | ref |
      | Login button  | e4  |
    Then the main window title changes to "Dashboard"
```

### Example 2: Error Handling - Stale Reference

```gherkin
Scenario: Handling stale element reference
    Given the application is attached
    And the AI agent has a valid snapshot with button [ref=e5]
    When the application UI changes (button removed by app logic)
    And the AI agent calls wpf_click with ref="e5"
    Then the response is:
      """json
      {
        "success": false,
        "error": {
          "code": "ELEMENT_STALE",
          "message": "Element ref 'e5' is no longer valid",
          "suggestion": "Call wpf_snapshot to refresh element references",
          "recoverable": true
        }
      }
      """
    When the AI agent calls wpf_snapshot
    Then a fresh element tree is returned with new references
```

### Example 3: DataGrid Navigation with Large Dataset

```gherkin
Scenario: Navigating a DataGrid with 1000 rows
    Given a DataGrid [ref=e10] with 1000 customer records
    When the AI agent calls wpf_snapshot with root_ref="e10"
    Then visible rows are returned (typically 15-30 based on height)

    When the AI agent calls wpf_scroll:
      | ref | direction | amount |
      | e10 | down      | page   |
    Then the response includes:
      """json
      {
        "success": true,
        "data": {"scroll_performed": true},
        "metadata": {"warnings": ["Previous element refs may be stale"]}
      }
      """

    When the AI agent calls wpf_snapshot with root_ref="e10"
    Then new rows are visible with fresh element refs
    And previous refs (e.g., e11, e12) are no longer valid
```

### Example 4: Modal Dialog Handling

```gherkin
Scenario: Handling a confirmation dialog
    Given the main window is visible
    When the AI agent calls wpf_click on "Delete" button
    And a confirmation dialog appears

    When the AI agent calls wpf_list_windows
    Then the response contains:
      """json
      {
        "success": true,
        "data": {
          "windows": [
            {"ref": "w1", "title": "Main Window", "is_active": false},
            {"ref": "w2", "title": "Confirm Delete", "is_active": true, "is_modal": true}
          ]
        }
      }
      """

    When the AI agent calls wpf_snapshot with focus on modal window
    Then the dialog elements are returned:
      """
      - window "Confirm Delete" [ref=e1] [modal]
        - text "Are you sure you want to delete?" [ref=e2]
        - button "Yes" [ref=e3]
        - button "No" [ref=e4]
      """

    When the AI agent calls wpf_click with ref="e3"
    Then the dialog closes
    And the main window becomes active
```
...
*Reviewed By: Wiegers (Requirements), Adzic (Examples), Fowler (API Design), Nygard (Reliability), Crispin (Testing)*
